package com.ll.mq.rabbitmq.config.mqtoken;
...
public class TransmitUserTokenFilter implements Filter {
    ...
    private void initUserInfo(HttpServletRequest request) {
        ...
        String queryToken = request.getParameter(X_ACCESS_TOKEN);
        if (token!=null) {
            try {
                //将Token放入当前线程上下文中
        ...
        }else if(StringUtils.isNotBlank(queryToken)){
            try {
                //将Token放入当前线程上下文中
                UserTokenContext.setToken(queryToken);
            } catch (Exception e) {
                //e.printStackTrace();
            }
        }
