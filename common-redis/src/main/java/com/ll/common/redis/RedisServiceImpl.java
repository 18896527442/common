...
public class RedisServiceImpl implements RedisService<String, Object> {
    ...
    public Long getExpireTime(String key){
        return this.redisTemplate.boundSetOps(key).getExpire();
    }
    ...
    public Set<Object> getSetUnion(String key, Set<Object> set) {
        return redisTemplate.boundSetOps(key).union(key);
    }
    ...
    public Set<Object> getSetIntersect(String key, Set<Object> set) {
        return redisTemplate.boundSetOps(key).intersect(key);
    }
    ...
    public boolean setnx(String key, Object value, Long second) {
        return redisTemplate.boundValueOps(key).setIfAbsent(value,second,TimeUnit.SECONDS);

//        return (Boolean) redisTemplate.execute(
//                (RedisCallback) conn -> {
//                    boolean flag = conn.setNX(key.getBytes(), value.getBytes());
//                    if (flag) {
...
//                });
    ...
    @Override
    public Long increment(String key, Long step, Long time){
        Boolean isExist= redisTemplate.hasKey(key);
        Long count=  redisTemplate.boundValueOps(key).increment(step);
        if(time!=null && !isExist) {
    ...
    @Override
    public Long deIncrement(String key, Long step, Long time){
        Boolean isExist= redisTemplate.hasKey(key);
        Long count=  redisTemplate.boundValueOps(key).decrement(step);
        if(time!=null && !isExist) {
            redisTemplate.boundValueOps(key).expire(time, TimeUnit.SECONDS);
        }
        return count;
